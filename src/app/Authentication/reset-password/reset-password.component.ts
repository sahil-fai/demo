...
export class ResetPasswordComponent implements OnInit {
  ...
  private createForm(){
    this.formReset = this._fb.group({
      username: [this._username, [ Validators.required ]],
      resetcode: [this._resetCode, [ Validators.required ]],
      // tslint:disable-next-line: max-line-length
      newpassword: ['', [ Validators.required, Validators.minLength(6), this.hasNumber, this.hasUppercase, this.hasLowercase, this.hasSpecialCharacter ]]
    });
  }
  ...
  public onReset(){
    ...
    this._authService.resetPassword(this.formReset.value).subscribe(res => {
